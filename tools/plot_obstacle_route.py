...
RUN_DIR = ROOT_DIR / "outputs" / "exp013_static_obstacle_detour_v1"
...
# World-file coordinates from exp013_obstacle_world.sdf.
# vehicle_blue starts at world pose (0.0, 2.0), while the controller
# records positions in vehicle_blue's local odometry frame.
BLUE_INITIAL_WORLD_X = 0.0
BLUE_INITIAL_WORLD_Y = 2.0

OBSTACLE_WORLD_CENTER_X = 3.5
OBSTACLE_WORLD_CENTER_Y = 2.0
OBSTACLE_SIZE_X = 1.4
OBSTACLE_SIZE_Y = 1.6
GOAL_TOLERANCE_M = 0.025

# Convert the static obstacle center into the same odometry frame used
# by global_route_trajectory.csv.
OBSTACLE_CENTER_X = OBSTACLE_WORLD_CENTER_X - BLUE_INITIAL_WORLD_X
OBSTACLE_CENTER_Y = OBSTACLE_WORLD_CENTER_Y - BLUE_INITIAL_WORLD_Y
...
def main():
    ...
    for row in trajectory_rows:
        ...
        waypoint_paths.setdefault(waypoint_name, []).append(row)

    route_plot_path = OUTPUT_DIR / "obstacle_detour_route_overview.png"
    error_plot_path = OUTPUT_DIR / "obstacle_detour_error_comparison.png"
    metrics_path = OUTPUT_DIR / "obstacle_detour_metrics.txt"

    plt.figure(figsize=(9, 7))

    obstacle = Rectangle(
        (
            OBSTACLE_CENTER_X - OBSTACLE_SIZE_X / 2.0,
            OBSTACLE_CENTER_Y - OBSTACLE_SIZE_Y / 2.0,
        ),
        OBSTACLE_SIZE_X,
        OBSTACLE_SIZE_Y,
        alpha=0.35,
    ...
    plt.xlabel("Odom X (m)")
    plt.ylabel("Odom Y (m)")
    plt.title("EXP-013: Fixed Odom-Frame Detour Route Around Static Obstacle")
    plt.grid(True)
    plt.axis("equal")
    ...
    plt.title("EXP-013: Obstacle Detour Waypoint Error")
    ...
    with metrics_path.open("w", encoding="utf-8") as file:
        file.write("EXP-013 Static Obstacle Detour Metrics\n")
        file.write("=======================================\n")
        file.write(f"Waypoint count: {len(summary_rows)}\n")
        file.write(f"Success count: {success_count}\n")
        file.write(
            f"Average final error: {average_error_mm:.3f} mm\n"
        )
        file.write(
            f"Maximum final error: {maximum_error_mm:.3f} mm\n"
        )
        file.write(
            f"Goal tolerance: {GOAL_TOLERANCE_M * 1000.0:.1f} mm\n"
        )
        file.write(f"Total route path length: {total_path_length:.3f} m\n")
        file.write(f"Total execution time: {total_duration:.2f} s\n")
        file.write("\n")
        file.write(
            "Experiment type: predefined waypoint detour in a static "
            "obstacle world; not autonomous obstacle detection or "
            "online path planning.\n"
        )

    print("EXP-013 可视化与指标汇总完成。")
    print()
    print(f"成功航点：{success_count} / {len(summary_rows)}")
