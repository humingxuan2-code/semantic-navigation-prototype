...
### EXP-015：Pose Goal Navigation：目标点位置与最终朝向对齐

实验目标：

- 在 EXP-014 的 A* 静态障碍物绕行基础上，进一步加入最终姿态目标；
- 每个航点不再只包含 `target_x` 和 `target_y`，而是加入 `target_yaw_deg`；
- 控制器先完成位置闭环跟踪，再在航点处进行最终 yaw 对齐；
- 验证机器人能否在静态障碍物场景中完成“到达目标位置 + 最终朝向对齐”；
- 明确本实验目标是 pose goal navigation，不是车道级超车回原车道任务。

路线文件：

`~/semantic_nav_ws/routes/exp015_pose_goal_route.csv`

Pose 控制器：

`~/semantic_nav_ws/tools/drive_blue_pose_route.cpp`

可视化与指标脚本：

`~/semantic_nav_ws/tools/summarize_exp015_pose_goal.py`

执行输出目录：

`~/semantic_nav_ws/outputs/exp015_pose_goal_v1/`
...
| 最大最终位置误差 | 19.90 mm |
| 平均最终位置误差 | 11.86 mm |
| 最大最终 yaw 误差 | 2.21 deg |
| 平均最终 yaw 误差 | 0.61 deg |
| 总执行时间 | 105.16 s |
...
| 项目 | x | y | yaw |
|---|---:|---:|---:|
| 目标值 | 5.800 | 2.000 | 0.00 deg |
| 实际值 | 5.797 | 2.003 | 1.15 deg |
...
`~/semantic_nav_ws/outputs/exp015_pose_goal_v1/pose_route_summary.csv`

`~/semantic_nav_ws/outputs/exp015_pose_goal_v1/pose_route_trajectory.csv`

`~/semantic_nav_ws/outputs/exp015_pose_goal_v1/summary/pose_goal_route_overview.png`

`~/semantic_nav_ws/outputs/exp015_pose_goal_v1/summary/pose_goal_error_summary.png`

`~/semantic_nav_ws/outputs/exp015_pose_goal_v1/summary/pose_goal_metrics.txt`

结论：

通过。

EXP-015 证明了当前控制器已经从单纯的 waypoint position tracking 扩展到 pose goal tracking：机器人能够先到达目标 `(x, y)`，再在目标点附近将最终车头朝向调整到指定 yaw 角度。

当前实验仍属于固定 odometry 坐标系下的 pose goal execution，尚未引入全局定位、动态障碍物、语义目标或在线重规划。

---

### EXP-016：基于机器人车身 Footprint 的轨迹安全评估

实验目标：

- 不再只根据最终航点误差判断实验是否成功；
- 使用机器人矩形 footprint 对整条执行轨迹进行安全性评估；
- 检查机器人车身与静态障碍物是否发生重叠或碰撞风险；
- 对比 EXP-014-v1、EXP-014-v2 和 EXP-015 的轨迹安全性；
- 量化说明“到点成功”不等于“轨迹安全”。

评估脚本：

`~/semantic_nav_ws/tools/evaluate_footprint_safety.py`

评估对象：

- `EXP-014-v1`：点机器人 A* 简化航点路线；
- `EXP-014-v2`：加入更大安全膨胀和更多中间航点后的 A* 路线；
- `EXP-015`：pose goal navigation 路线。

评估参数：

| 参数 | 数值 |
|---|---:|
| robot footprint length | 2.20 m |
| robot footprint width | 1.80 m |
| obstacle center in odom frame | (3.5, 0.0) |
| obstacle size | (1.4, 1.6) |

实验结果：

| 实验 | 轨迹点数 | 轨迹长度 | 最小 footprint-obstacle 距离 | 重叠 / 碰撞风险采样数 | 状态 |
|---|---:|---:|---:|---:|---|
| EXP-014-v1 point A* | 942 | 6.051 m | 0.00 mm | 266 | collision_risk |
| EXP-014-v2 inflated A* | 1523 | 7.179 m | 556.88 mm | 0 | safe |
| EXP-015 pose goal | 778 | 7.208 m | 554.26 mm | 0 | safe |

生成文件：

`~/semantic_nav_ws/outputs/exp016_footprint_safety_v1/footprint_safety_summary.csv`

`~/semantic_nav_ws/outputs/exp016_footprint_safety_v1/summary/footprint_safety_comparison.png`

`~/semantic_nav_ws/outputs/exp016_footprint_safety_v1/summary/footprint_safety_metrics.txt`

结论：

通过。

EXP-016 证明了最终航点误差并不足以单独判断导航实验是否安全。EXP-014-v1 虽然最终到点成功，但 footprint 评估检测到 266 个轨迹采样点存在车身与障碍物重叠风险，因此应判定为 collision risk。EXP-014-v2 和 EXP-015 在更大安全膨胀和更合理航点设置下均保持 0 个重叠采样点，最小安全距离约为 0.55 m，轨迹安全性明显更好。

当前评估仍是离线轨迹安全分析，尚未接入在线碰撞检测、costmap、动态障碍物或实时重规划。
